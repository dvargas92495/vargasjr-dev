    ...
    const makeRequest = (requestOptions, attempt = 1) => {
      return new Promise((resolve, reject) => {
        const req = https.request(requestOptions, (res) => {
          ...
          res.on("end", () => {
            console.log(`Webhook response (attempt ${attempt}):`, res.statusCode, data);
            
            if (res.statusCode === 308) {
              console.log("308 Redirect detected! Response headers:", JSON.stringify(res.headers, null, 2));
              console.log("Request URL was:", `https://${requestOptions.hostname}${requestOptions.path}`);
              
              if (attempt === 1) {
                console.log("Retrying with trailing slash...");
                const retryOptions = {
                  ...requestOptions,
                  path: requestOptions.path.endsWith('/') ? requestOptions.path : requestOptions.path + '/'
                };
                ...
                const retryOptions = {
                  ...requestOptions,
                  path: requestOptions.path.endsWith('/') ? requestOptions.path.slice(0, -1) : requestOptions.path
                };
