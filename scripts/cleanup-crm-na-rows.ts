...
class CleanupCrmNaRowsMigration extends OneTimeMigrationRunner {
  ...
  protected async runMigration(): Promise<void> {
    ...
    const rowsToDelete = await db
      ...
      .from(ContactsTable)
      .where(
        and(eq(ContactsTable.fullName, "N/A"), eq(ContactsTable.email, "N/A"))
      );
    ...
    if (this.isPreviewMode) {
      this.logWarning(
        `PREVIEW MODE: Would delete ${rowCount} rows from ContactsTable where both fullName='N/A' and email='N/A'`
      );
      ...
      this.logSuccess(
        `Successfully deleted ${rowCount} rows from ContactsTable`
      );
    }
  }
}
