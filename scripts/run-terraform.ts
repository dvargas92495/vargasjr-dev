...
class TerraformRunner {
  ...
  async run(): Promise<void> {
    console.log(
      `🔍 ${this.isPreviewMode ? "Previewing" : "Running"} Terraform...`
    );
  ...
  private async formatAndPostPlanResults(planOutput: string): Promise<void> {
    ...
    await this.postComment(
      commentContent,
      "Posted Terraform plan comment to PR"
    );
  }
  ...
  private async handlePlanError(errorOutput: string): Promise<void> {
    ...
    await this.postComment(
      commentContent,
      "Posted Terraform plan error comment to PR"
    );
  }
