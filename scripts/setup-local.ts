  ...
  private async setupLocalDatabase(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        try {
          execSync(`psql postgresql://postgres:password@localhost:5432/postgres -c "SELECT 1;"`, { stdio: 'pipe' });
          console.log('✅ PostgreSQL server connection successful.');
          
          execSync(`psql postgresql://postgres:password@localhost:5432/postgres -c "CREATE DATABASE ${dbName};"`, { stdio: 'pipe' });
          console.log(`✅ Created database: ${dbName}`);
      ...
      console.log('\n💡 You may need to:');
      console.log('   1. Start PostgreSQL service: sudo systemctl start postgresql');
      console.log('   2. Set a password for the postgres user: sudo -u postgres psql -c "ALTER USER postgres PASSWORD \'password\';"');
      console.log('   3. Manually create the database: sudo -u postgres createdb vargasjr_dev');
  ...
  private async installAndSetupPostgres(): Promise<void> {
    ...
    try {
      if (platform === 'darwin') {
        ...
        console.log('❌ Unsupported platform. Please install PostgreSQL manually.');
  ...
  private async setupPostgresLinux(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.log('❌ Could not detect package manager. Please install PostgreSQL manually.');
        process.exit(1);
      }
    }
    ...
    try {
      execSync('sudo -u postgres psql -c "ALTER USER postgres PASSWORD \'password\';"', { stdio: 'inherit' });
    } catch (error) {
      console.log('⚠️  Could not set postgres password automatically.');
      console.log('Please set it manually: sudo -u postgres psql -c "ALTER USER postgres PASSWORD \'password\';"');
    }
