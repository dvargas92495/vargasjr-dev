...
async function runHealthcheck(): Promise<void> {
  try {
    ...
    console.log(
      `Agent Environment: ${healthData.environment.agentEnvironment}`
    );
    console.log(`PR Number: ${healthData.environment.prNumber}`);
    console.log(
      `Working Directory: ${healthData.environment.workingDirectory}`
    );
    console.log(`Node Version: ${healthData.environment.nodeVersion}`);
    console.log("");

    console.log("--- Environment Variables Check ---");
    Object.entries(healthData.environmentVariables.critical).forEach(
      ([varName, isSet]) => {
        console.log(`${varName}: ${isSet ? "✓ Set" : "✗ Missing"}`);
      }
    );

    Object.entries(healthData.environmentVariables.optional).forEach(
      ([varName, isSet]) => {
    ...
    if (healthData.processes.agentProcesses.length > 0) {
      ...
      healthData.processes.agentProcesses.forEach((process) =>
        console.log(`  ${process}`)
      );
    } else {
    ...
    if (healthData.logs.errorLog?.exists) {
      if (healthData.logs.errorLog.empty) {
        ...
        console.log(
          `Failed to read error.log: ${healthData.logs.errorLog.error}`
        );
      } else {
    ...
    if (healthData.logs.browserErrorLog?.exists) {
      if (healthData.logs.browserErrorLog.empty) {
        ...
        console.log(
          `Failed to read browser-error.log: ${healthData.logs.browserErrorLog.error}`
        );
      } else {
        console.log(
          `Browser Error Log (${healthData.logs.browserErrorLog.length} chars):`
        );
    ...
    if (healthData.logs.agentLog?.exists) {
      if (healthData.logs.agentLog.empty) {
        ...
        console.log(
          `Failed to read agent.log: ${healthData.logs.agentLog.error}`
        );
      } else {
    ...
    console.log(
      `Detailed report saved to memory (${healthData.detailedReport.length} chars)`
    );
