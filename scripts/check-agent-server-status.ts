...
class CheckAgentServerStatusMigration extends OneTimeMigrationRunner {
  ...
  protected async runMigration(): Promise<void> {
    ...
    if (this.isPreviewMode) {
      this.logSuccess("Preview mode: Would execute SSH commands to check agent server status");
  ...
  private async previewCommands(): Promise<void> {
    ...
    previewContent += "The following SSH commands would be executed on the production instance:\n\n";
    ...
    previewContent += "**Target Instance**: Production (tags: Name=vargas-jr, Type=main)\n";
