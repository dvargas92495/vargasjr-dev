...
import { cleanEmailContent, parseMimeEmail } from "@/server/email-content-parser";
  ...
  try {
    ...
    try {
      ...
      if (s3Response.Body) {
        ...
        const mimeProcessedBody = parseMimeEmail(rawEmailBody);
        emailBody = cleanEmailContent(mimeProcessedBody);
      }
    } catch (error) {
