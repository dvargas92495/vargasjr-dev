...
const EditCronModal = forwardRef<
  EditCronModalHandle,
  EditCronModalProps
>(({ currentCronExpression, onSave }, ref) => {
    ...
    try {
      ...
      if (useNaturalLanguage && scheduleDescription.trim()) {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || "Failed to convert schedule description");
        }
      ...
      if (!finalCronExpression.trim()) {
        throw new Error("Please provide either a schedule description or cron expression");
      }
  ...
  }, [useNaturalLanguage, scheduleDescription, cronExpression, onSave, closeModal]);
