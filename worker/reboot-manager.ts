...
export async function getLatestVersion(): Promise<string | null> {
  ...
  try {
    ...
    if (typeof latestVersion !== "string") {
      console.log(
        `Failed to parse release data for tag name: ${JSON.stringify(
          releaseData
        )}`
      );
      return null;
...
export async function rebootAgent(
  ...
  try {
    ...
    try {
      execSync("yes | rm -rf ~/.cache/pypoetry/virtualenvs/*", {
        stdio: "inherit",
      });
    ...
    try {
      execSync(`tar -xzf vargasjr_dev_agent-${targetVersion}.tar.gz`, {
        stdio: "inherit",
      });
