...
class GetPlaidTransactions(BaseNode):
    class Outputs(BaseNode.Outputs):
        transactions: List[PersonalTransaction]
        snapshot: float

    def run(self) -> Outputs:
        plaid_app = get_application_by_name("Family Plaid Account")
        if not plaid_app:
            raise ValueError("Family Plaid Account application not found in database")
        
        if not plaid_app.client_id or not plaid_app.client_secret:
            raise ValueError("Plaid client credentials not configured in database")

        if not plaid_app.access_token:
            raise ValueError("Plaid access token not configured in database")

        headers = {
            "PLAID-CLIENT-ID": plaid_app.client_id,
            "PLAID-SECRET": plaid_app.client_secret,
            "Content-Type": "application/json"
        }
        base_url = "https://production.plaid.com"
        
        accounts_payload = {
            "access_token": plaid_app.access_token
        }
        ...
        for transaction in transactions_data.get("added", []):
            ...
            all_transactions.append(
                PersonalTransaction(
                    ...
                    description=transaction.get("name", "Plaid Transaction"),
                    amount=amount,
