...
def _is_function_call() -> LazyReference:
    return LazyReference(lambda: TriageMessageNode.Outputs.results[0]["type"].equals("FUNCTION_CALL"))  # type: ignore
...
class ParseFunctionCallNode(BaseNode):
    class Ports(BaseNode.Ports):
        ...
        no_action = Port.on_if(
            _is_function_call()
            & LazyReference(lambda: ParseFunctionCallNode.Outputs.action.equals("no_action"))  # type: ignore
        )
        ...
        email_reply = Port.on_if(
            _is_function_call()
            & LazyReference(lambda: ParseFunctionCallNode.Outputs.action.equals("email_reply"))  # type: ignore
        )
        email_initiate = Port.on_if(
            _is_function_call()
            & LazyReference(lambda: ParseFunctionCallNode.Outputs.action.equals("email_initiate"))  # type: ignore
        )
        text_reply = Port.on_if(
            _is_function_call()
            & LazyReference(lambda: ParseFunctionCallNode.Outputs.action.equals("text_reply"))  # type: ignore
        )
        slack_reply = Port.on_if(
            _is_function_call()
            & LazyReference(lambda: ParseFunctionCallNode.Outputs.action.equals("slack_reply"))  # type: ignore
        )
